...
def usersLogIn():
    if request.method == 'POST':
        ...
        if form_id == "login": #if login
            ...
            if not str.isnumeric(login_id) and not login_id == "":
...
def schedule():
    ...
    if current_user.prefinal == True:
        ...
        paymentname = "prelim"
    return render_template('schedule.html',login_id=login_id,current_user=current_user,tuition = tuition,name=paymentname)
...
def delete_record(id):
    record = Payment.query.get(id)
    if not record:
        return jsonify({'status': 'error', 'message': 'Record not found'}), 404
    current_user = User.query.filter_by(id=int(record.student_id)).first()
    paymentname = record.name
    if paymentname == "finals":
        current_user.finals = False
    elif paymentname == "prefinal":
        current_user.prefinal = False
    elif paymentname == "midterm":
        current_user.midterm = False
    elif paymentname == "prelim":
        current_user.prelim = False
    db.session.commit()
